...
/**
 * Public list CMS pages (published only).
 * Used by storefront footer/menu to discover available pages.
 */
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const defaultLocale =
    typeof settings.default_locale === "string" ? settings.default_locale : "vi"
  const qLocale =
  ...
  const rows = await cms.listStoreCmsPages(
  ...
  const pages = rows
    ...
    .map((p) => ({
      slug: p.slug,
      title: resolveCmsPageI18nField(p.title, locale, enabled, defaultLocale),
    }))
