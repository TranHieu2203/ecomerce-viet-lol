...
- Chỉ **hai locale** `vi` + `en` trong phạm vi sản phẩm; fallback nội dung i18n theo **`architecture.md`** (thường fallback `vi` nếu thiếu `en`).
...
- Dữ liệu đa ngôn ngữ CMS/banner: JSON có key **`vi` / `en`**; catalog mở rộng: **`metadata.i18n`** như `architecture.md` — không tự ý thêm locale thứ ba trong phạm vi hiện tại.
